...
def generate_clothoid_paths(start_point, start_yaw_list, goal_point,
                            goal_yaw_list, n_path_points):
    """注释
    ...
    (yaw) at goal point.这个函数列表生成从起始位置到终点位置各个方向的多种曲线

    :param start_point: Start point of the path     起始点
    :param start_yaw_list: Orientation list at start point in radian    起始点朝向列表
    :param goal_point: Goal point of the path       终点
    :param goal_yaw_list: Orientation list at goal point in radian  终点朝向列表
    :param n_path_points: number of path points     路径点数
    ...
    for start_yaw in start_yaw_list:
        for goal_yaw in goal_yaw_list:
            ...
            clothoid = generate_clothoid_path(start_point, start_yaw,
                                              goal_point, goal_yaw,
                                              n_path_points)
            # 将生成的 clothoid 路径添加到 clothoids 列表中
            clothoids.append(clothoid)
    # 返回生成的 clothoids 列表
    return clothoids


def generate_clothoid_path(start_point, start_yaw, goal_point, goal_yaw,
                           n_path_points):
    """
...
def main():
    ...
    num_path_points = 100
    clothoid_paths = generate_clothoid_paths(start_point,
                                             start_orientation_list,
                                             goal_point, goal_orientation_list,
                                             num_path_points)
    if show_animation:
        draw_clothoids(start_point,
                       goal_point,
                       num_path_points,
                       clothoid_paths,
                       save_animation=False)
